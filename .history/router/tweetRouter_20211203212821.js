...
router.get("/:id", (req, res, next) => {
  ...
  if (!foundTweets) {
    return res.status(404).json({message:"Ooooooooops It's not ID check id Pleaaaaaase"});
  }
...
router.post("/", (req,res,next)=>{
    ...
    const newTweet = {
        id:new Date().toString(), text, name, username, url, createdAt: Date.now().toString()
    }
})
